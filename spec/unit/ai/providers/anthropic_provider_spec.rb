...
RSpec.describe Taskmate::AI::Providers::AnthropicProvider do
  ...
  describe "#complete" do
    let(:success_body) do
      JSON.generate({ "content" => [{ "text" => "Anthropic result" }] })
    end

    it "returns extracted content on 200" do
      stub_post("/v1/messages", status: 200, body: success_body)
      expect(provider.complete(prompt: "Do something", skill_id: "test")).to eq("Anthropic result")
    end

    it "raises AiAuthError on 401" do
      stub_post("/v1/messages", status: 401, body: '{"error":"unauthorized"}')
  ...
  describe "initialization" do
    it "raises AiAuthError when API key env var is absent" do
      without_env("TASKMATE_ANTHROPIC_API_KEY") do
        expect { described_class.new }
          .to raise_error(Taskmate::AiAuthError, /TASKMATE_ANTHROPIC_API_KEY/)
      end
    end
  end
end

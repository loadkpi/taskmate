  ...
  module Jira
    class MarkdownValidator
      ...
      UNSUPPORTED = [
        ...
        { pattern: /^={2,}\s*$/,    feature: "setext_heading",
          message: "Setext-style headings (===) are not supported" },
        { pattern: /^-+\s*$/,       feature: "setext_heading",
          message: "Setext-style headings (---) as header is not supported" },
        { pattern: /^\s{4}/,        feature: "indented_code",
          message: "Indented code blocks (4 spaces) are not supported; use fenced ``` blocks" },
      ...
      def validate(markdown)
        ...
        markdown.to_s.lines.each_with_index do |line, idx|
          ...
          UNSUPPORTED.each do |rule|
            ...
            if (rule[:feature] == "setext_heading") && prev_line.strip.empty?

      ...
      class << self
        ...
        def build_ai(ai, env:)
          provider = env.fetch("TASKMATE_AI_PROVIDER",
                               (ai["provider"] || ai["default_provider"] || "disabled").to_s)
          model    = env.fetch("TASKMATE_AI_MODEL",
                               (ai["model"] || ai["default_model"] || "").to_s)
          enabled  = ai.fetch("enabled", true)
        ...
        def build_security(s)
          SecurityConfig.new(
            require_consent_for_ai:   s.fetch("require_consent_for_ai", true),
            require_confirm_for_push: s.fetch("require_confirm_for_push", true),
            secret_detection:         s.fetch("secret_detection", true)
          )
        end

        def build_push(p)
          fields = Array(p["allowed_fields"])
